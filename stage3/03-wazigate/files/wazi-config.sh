...
if [ $(id -u) -ne 0 ]; then
  printf "Script must be run as root. Try 'sudo wazi-config'\n"
  exit 1
fi
...
calc_wt_size
while true; do
  FUN=$(whiptail --title "WaziGate Software Configuration Tool (wazi-config)" --menu "Setup Options" $WT_HEIGHT $WT_WIDTH $WT_MENU_HEIGHT --cancel-button Exit --ok-button Select \
